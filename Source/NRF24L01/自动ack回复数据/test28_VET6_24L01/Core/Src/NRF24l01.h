...
#define NOP             0xFF  //空操作,可以用来读状态寄存器	 
	...
	#define WR_ACK_PAYLOAD	0xA8//+ppp	//RX模式下,通过PIPE PPP将数据通过ACK形式发送，最多允许三帧数据存在于FIFO。
	#define RD_RX_PL_WID		0x60	//读RX FIFO顶层R_RX_PLOAD载荷长度,若超出32字节则清除RX FIFO
	#define Activate			0x50//写入0x73激活R_RX_PL_WID,W_ACK_PAYLOAD,W_TX_PAYLOAD_NOACK寄存器,再次写入禁用.只能在省电模式和掉电模式执行
...
#define Clr_NRF24L01_CSN   \
	HAL_GPIO_WritePin(CS_24L01_GPIO_Port, CS_24L01_Pin, GPIO_PIN_RESET)
#define Set_NRF24L01_CSN   \
	HAL_GPIO_WritePin(CS_24L01_GPIO_Port, CS_24L01_Pin, GPIO_PIN_SET)

//NRF2401_IRQ数据输入
#define READ_NRF24L01_IRQ  \
	HAL_GPIO_ReadPin(INT_24L01_GPIO_Port, INT_24L01_Pin)
