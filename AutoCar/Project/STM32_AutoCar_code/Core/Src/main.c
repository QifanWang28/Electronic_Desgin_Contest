  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
		...
		switch(car_case)
		{
			case 0:
			{
				if(L2 != 1 || L1 != 1 || Center != 1 || R1 != 1 || R2 != 1)
				{
					car_case = 1;
					set_motor_disable();
					set_motor2_disable();
			...
			case 1:
			{
				...
				if((L2 == 1 && L1 == 1 && Center == 1 && R1 == 1 && R2 == 1) && target_1_R == 5)
				{
					loop[count_loop++] = 3;
					car_case = 2;
					go_line = 0; 
			...
			case 2:
			{
				if(spin_sig == 0) spin_Turn(right_90);
				GetMotorLength();
				if(g_length >= 0.23 * 3.1416 * LUN_JU)
				{
					if(Center == 1)
					{
						car_case = 3;
			...
			case 3:
			{
				if(go_line == 0) Car_go_straight();
				if(L2 == 0 && L1 == 0 && Center == 0 && R1 == 0 && R2 == 0)
				{
					car_case = 4;
					go_line = 0;
					g_lMotorPulseSigma =0;
			...
			case 4:
			{
				...
				if(trans_sig == 0)
				{
					HAL_UART_Transmit(&huart3,loop,sizeof(loop),10000);
					trans_sig = 1;
				}
			}break;
	}
