    ...
    switch (cmd_type)
    {
      ...
      case SET_P_I_D_CMD:
      {
        ...
        d_temp = *(float *)&temp2;
				...
				/*要调那个PID就取消哪个注释*/
        //set_p_i_d(&pid_location2, p_temp, i_temp, d_temp);    // 设置哪个改那个，只能通过通道观察
				set_p_i_d(&line_following, p_temp, i_temp, d_temp);
      ...
      case SET_TARGET_CMD:
      {
		    ...
		    // set_point = 60;
				/*要调那个PID就取消哪个注释*/
				//set_pid_target(&line_following, set_point);     //这个set_point值应该是脉冲数
...
//        __HAL_TIM_SET_AUTORELOAD(&htim7,temp);                             // 设置定时器周期1~1000ms
